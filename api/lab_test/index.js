...
router
	.route('/')
	.get(async (req, res) => {
		const lab_tests = await db.query('views/pending_tests');
		res.send(lab_tests.rows.map((lab_test) => {
            delete lab_test.value.doctype;
            return lab_test.value;
        }));
	})
